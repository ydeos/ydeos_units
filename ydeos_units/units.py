...
def _create_code(dict_name: str, key_name: str) -> str:
    r"""Create the code to be compiled on the fly by the conversion modules.

    Parameters
    ----------
    dict_name : The name of the dictionary that defines
                the units in the conversion module
    key_name : The key name in the dictionary that defines the units
...
for k in areas:
    exec(_create_code("areas", k), globals())


distances = {"mm": 1e-3, "millimeter": 1e-3, "millimeters": 1e-3,
             "millimetre": 1e-3, "millimetres": 1e-3,
             # cm
             "cm": 1e-2, "centimeter": 1e-2, "centimeters": 1e-2,
             "centimetre": 1e-2, "centimetres": 1e-2,
             # m
...
for k in distances:
    # code = fs_units.base._create_code("distances", k)
    # exec code in module.__dict__
    # g = globals()
    exec(_create_code("distances", k), globals())
...
pressures = {"Pa": 1., "pascal": 1., "pascals": 1.,
             ...
             "mmHg": 133.322387415, "millimeter_of_mercury": 133.322387415,
             "millimeters_of_mercury": 133.322387415}

for k in pressures:
    exec(_create_code("pressures", k), globals())


speeds = {"ms": 1., "mps": 1., "meter_per_second": 1., "meters_per_second": 1.,
          # km / h
          "kmh": 1000. / 3600., "kilometer_per_hour": 1000. / 3600.,
          "kilometers_per_hour": 1000. / 3600.,
          # knots
          "kt": 1852. / 3600., "knt": 1852. / 3600., "knts": 1852. / 3600.,
          "knot": 1852. / 3600., "knots": 1852. / 3600.}

for k in speeds:
    exec(_create_code("speeds", k), globals())


torques = {"Nm": 1., "newton_metre": 1., "newton_metres": 1.,
           "newton_meter": 1., "newton_meters": 1.}

for k in torques:
    exec(_create_code("torques", k), globals())


volumes = {"m3": 1., "cubic_meter": 1., "cubic_meters": 1.,
           # litres
           "l": 0.001, "litre": 0.001, "liter": 0.001, "litres": 0.001,
           "liters": 0.001,
           # cm3
           "cm3": 1e-6, "centimeter_cube": 1e-6, "centimeters_cube": 1e-6,
           # ml
           "ml": 1e-6, "millilitre": 1e-6, "millilitres": 1e-6,
           "milliliter": 1e-6, "milliliters": 1e-6,
           # mm3
           "mm3": 1e-9, "millimeter_cube": 1e-9, "millimeters_cube": 1e-9,
           # pint
           "pt": 568.26125 * 1e-6, "pint": 568.26125 * 1e-6,
           "pints": 568.26125 * 1e-6,
           # quart
           "qt": 1136.5225 * 1e-6, "quart": 1136.5225 * 1e-6,
           "quarts": 1136.5225 * 1e-6,
           # gallon
           "gal": 4546.09 * 1e-6, "gallon": 4546.09 * 1e-6,
           "gallons": 4546.09 * 1e-6}

for k in volumes:
    exec(_create_code("volumes", k), globals())


def convert(value: Union[int, float], to_unit: str, from_unit: str) -> float:
    r"""Convert a value from from_unit to to_unit.

    Convenience function for cases where the to_unit and the from_unit
    are in string form.
...
def decimal_degrees(degrees: Union[int, float] = 0,
    ...
    r"""Convert to decimal degrees.

    Convert degrees and decimal minutes or degrees minutes seconds
    to decimal degrees
...
def degrees_minutes_seconds(decimal_degree: float = 0.) -> Tuple[int, int, float, float]:
    r"""Convert to degrees minutes seconds.

    Convert decimal degrees to degrees and decimal minutes
    and degrees minutes seconds

    Returns a tuple (int, int, float, float):
    degrees, minutes, decimal_minutes (e.g. 30.99), seconds
...
def convert_imperial_str(value: str, to_unit: str):
    r"""Convert an imperial distance into a metric distance.

    Convert a distance expressed in the imperial system
...
def convert_imperial(feet: float = 0.,
                     inches: float = 0.,
                     to_unit: str = "mm") -> float:
    r"""Convert imperial distance.
    ...
    """
    return (convert(feet, from_unit="feet", to_unit=to_unit)
            + convert(inches, from_unit="inches", to_unit=to_unit))
...
def fahrenheit(**kwargs):
    ...
    if list(kwargs.keys())[0] in ["kelvin", "kelvins", "K"]:
